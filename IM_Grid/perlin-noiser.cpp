...
// Defines the range of values of the generated noise.
// The values obtained in noising are scaled to fit in this range, so there is certainly
// at least one value equal to lower_limit and at least one equal to upper_limit
// in the output of generateNoise method.
...
// Generates Perlin noise for given coordinates. It takes points as input and saves
// the results in values; values[i][j] refers to points[i][j].
// The output should be empty (it will be cleared and recreated anyway).
// @param points: grid of coordinates to generate noise for
// @param values: grid for values
